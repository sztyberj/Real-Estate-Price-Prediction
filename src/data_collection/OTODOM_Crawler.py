...
class OfferCrawler:
    ...
    def _yes_no(lst, key) -> str:
        return "Tak" if OfferCrawler._get_characteristic(lst, key) else "Nie"

    @staticmethod
    def _compose_floor(lst) -> str:
        fl  = OfferCrawler._get_characteristic(lst, "floor_no", "localizedValue")
        tot = OfferCrawler._get_characteristic(lst, "building_floors_num")
        return f"{fl}/{tot}" if fl and tot else fl or ""

    @staticmethod
    def _extract_next_data(html: str):
        m = OfferCrawler.JSON_RE.search(html)
        if not m:
    ...
    def parse(url: str) -> Optional[PropertyRecord]:
        try:
            ...
            data = OfferCrawler._extract_next_data(resp.text)
            ...
            if isinstance(ad_price, dict):
                ...
                price = str(ad_price)
            if not price:
                price = OfferCrawler._get_characteristic(char, "price")
            ...
            return PropertyRecord(
                price            = price,
                district         = district,
                price_per_meter  = OfferCrawler._get_characteristic(char,"price_per_m"),
                area             = OfferCrawler._get_characteristic(char,"m"),
                rooms            = OfferCrawler._get_characteristic(char,"rooms_num"),
                floor            = OfferCrawler._compose_floor(char),
                market_type      = "pierwotny" if ad.get("market")=="PRIMARY" else "wtórny",
                furnished        = OfferCrawler._yes_no(char,"equipment_types"),
                description      = ad.get("description",""),
                date             = ad.get("createdAt","")[:10],
                url              = url,
                title            = ad.get("title",""),
                building_type    = OfferCrawler._get_characteristic(char,"building_type"),
                year_built       = OfferCrawler._get_characteristic(char,"build_year"),
                rent             = OfferCrawler._get_characteristic(char,"rent"),
                finish_status    = OfferCrawler._get_characteristic(char,"construction_status"),
                ownership        = OfferCrawler._get_characteristic(char,"building_ownership"),
                heating          = OfferCrawler._get_characteristic(char,"heating","localizedValue"),
                elevator         = OfferCrawler._yes_no(char,"lift"),
                ad_id            = str(ad.get("id","")),
...
class OtodomCrawler:
    ...
    def _scrape_buffer(self, links: list[str], page_marker: int):
        ...
        with ThreadPoolExecutor(max_workers=self.workers) as ex:
            futures = [ex.submit(OfferCrawler.fetch_offer, u) for u in links]
            fut_iter = futures
            if tqdm:
    ...
    def run(self):
        ...
        buffer: list[str] = []
        for p in page_iter:
            links = ListingCrawler.fetch_listing(p)
            buffer.extend(links)
...
if __name__ == "__main__":
    ...
    OtodomCrawler(cfg.start_page, cfg.max_pages, cfg.workers).run()
    """
    OtodomCrawler().run()